...
typedef struct
{
	...
	__vo uint32_t RTSR;				// Address offset :
	__vo uint32_t FTSR;				// Address offset :
	__vo uint32_t SWIER;
	__vo uint32_t PR;
...
#define GPIOA_REG_RESET()			do { (RCC->AHB1RSTR |= (1<<0));(RCC->AHB1RSTR &= ~(1<<0)); }while(0) // do while condition zero loop : this is technique in 'c' programming to execute multiple 'c' statements using single 'c' macro
...
#define GPIO_BASEADDR_TO_CODE(x)		((x == GPIOA)?0:(x == GPIOB)?1:(x == GPIOC)?2:(x == GPIOD)?3:(x == GPIOE)?4:(x == GPIOF)?5:(x == GPIOG)?6:(x == GPIOH)?7:(x == GPIOI)?8:(x == GPIOJ)?9:(x == GPIOK)?10:0)

/**************************************************************************************************************************************
 *  Bit posotion definitions of I2C peripheral
 **************************************************************************************************************************************/
/*
